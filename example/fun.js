...
Array.prototype.foldl = function (fn, init) {
    ...
    var val = arguments.length < 2 ? this[0] : init,
        i   = arguments.length < 2 ? 1 : 0;
    for (; i < this.length; i++) {
        val = fn(val, this[i]);
    }
    return val;
 ...
 *
 * @param fn     binary iterator for left fold
 * @param [init] optional starting value. If not given the array its last
 *               value will be used
 */
Array.prototype.foldr = function (fn, init) {
    ...
    var val = arguments.length < 2 ? this[this.length-1] : init,
        i   = arguments.length < 2 ? this.length-1 : this.length;
...
/**
 * computes the array its intersecton with another array. If the array contains
...
/**
 * computes the array its union with another array. If the array contains
 * duplicates, so will the result.
 * All elements in both arrays are compared using the strict equality comparator ===.
 *
 * @param {Array} arr Array to compute the union with.
 *
 * @return new Array with all elements present in this and given Array.
 */
